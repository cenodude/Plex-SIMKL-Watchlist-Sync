# _watchlist.py
# Watchlist logic for Plex ⇄ SIMKL Web UI (PlexAPI-only)
...
def build_watchlist(state: Dict[str, Any], tmdb_api_key_present: bool) -> List[Dict[str, Any]]:
    ...
    The TMDb key is not required here; posters are handled elsewhere.
    """
...
def _guid_variants_from_key_or_item(key: str, item: Optional[Dict[str, Any]] = None) -> List[str]:
    ...
    Examples for imdb:tt123 → ["imdb://tt123", "com.plexapp.agents.imdb://tt123", "com.plexapp.agents.imdb://tt123?lang=en"]
    """
...
def _extract_plex_identifiers(item: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
    ...
    Only the GUID is used for matching with PlexAPI; ratingKey is ignored in the PlexAPI-only path.
    """
...
def delete_watchlist_item(key: str, state_path: Path, cfg: Dict[str, Any], log=None) -> Dict[str, Any]:
    """
    Remove a single item from the user's Plex *online* watchlist using python-plexapi.

    Behavior
    --------
    - Matches the item by GUID against the user's online watchlist.
    - Removes only the Plex-side entry from state.json (SIMKL entries are preserved).
    - Returns a JSON-like dict with { ok: bool, ... } suitable for direct API responses.

    Requirements
    ------------
    - 'plexapi' must be installed.
    - A valid Plex account token must be present in cfg['plex']['account_token'].
    - For managed/secondary users, ensure the token has permission to mutate the account watchlist.
    """
    try:
        ...
        # Prefer Plex-side item to get better identifier hints; fall back to SIMKL-side.
        ...
        # Build GUID candidates to match against PlexAPI results
        guid, _ = _extract_plex_identifiers(item)
        variants = _guid_variants_from_key_or_item(key, item)
        if guid:
            # Ensure any state-provided guid is also considered.
            variants = list(dict.fromkeys(variants + [guid]))

        # Load online watchlist and find the item by normalized GUID
        ...
        for media in watchlist:
            ...
            # Some plexapi builds expose additional provider GUIDs via .guids
            try:
        ...
        # Remove and persist state changes (Plex-side only)
        account.removeFromWatchlist([found])

        if key in plex_items:
            del plex_items[key]
            state.setdefault("plex", {}).setdefault("items", {})
            state["plex"]["items"] = plex_items
