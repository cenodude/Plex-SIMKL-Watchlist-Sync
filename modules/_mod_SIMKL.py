...
class _LoggerAdapter:
    """Tiny adapter so this module accepts your _logging.Logger or a callable.
    Avoids Optional attribute access by always holding a concrete logger.
    """
    ...
    def __call__(
        ...
        *,
        ...
        # Prefer structured logger methods if available
...
class SIMKLModule(SyncModule):
    ...
    # config checks used by sync-only work
    def validate_config(self) -> None:
        sid = self._simkl.get("client_id", "")
        at  = self._simkl.get("access_token", "")
        rt  = self._simkl.get("refresh_token", "")
        if not sid:
    ...
    def simkl_remove_from_history(self, items_by_type: Mapping[str, List[Mapping[str, Any]]], *, dry_run: bool = False) -> Dict[str, Any]:
        """Remove items from PTW/history. items_by_type = {"movies":[{"ids":{...}}], "shows":[{"ids":{...}}]}"""
        self._ensure_token()
        payload: Dict[str, List[Dict[str, Any]]] = {}
        for typ in ("movies", "shows"):
            rows = []
    ...
    def run_sync(self, ctx: SyncContext) -> SyncResult:
        ...
        try:
            ...
            self._log(f"SIMKL PTW total={items_total}", level="INFO")
